...
namespace Microliu.Core.RedisCache
{
    public class CacheService : ICacheService
    {
        ...
        {
            if (!_options.Startup) return false;
            var redis = GetRedisConnection();
            try
            {
                if (redis != null && redis.IsConnected)
        ...
        public List<string> StringMultiGet(params string[] keys)
        {
            ...
            if (!_options.Startup) return list;
            var redis = GetRedisConnection();
            try
            {
                if (redis != null && redis.IsConnected)
        ...
        {
            if (!_options.Startup) return default(T);
        ...
        {
            if (!_options.Startup) return null;
        ...
        public Dictionary<string, object> HashGetAll(string key)
        {
            ...
            if (!_options.Startup) return dic;
            var redis = GetRedisConnection();
            try
            {
                if (redis != null && redis.IsConnected)
        ...
        public string[] HashKeys(string key)
        {
            ...
            if (!_options.Startup) return new string[] { };
            var redis = GetRedisConnection();
            try
            {
                if (redis != null && redis.IsConnected)
        ...
        {
            _logger.Info($"[{DateTimeOffset.Now.ToString("yyyy-MM-dd HH:mm:ss.fff")}] [Redis连接成功] [{e.EndPoint}] [{e.Exception?.Message??""}]", new string[] { "redis", "connection", "restored" });
        }
