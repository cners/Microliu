...
namespace Microliu.FileService.Application
{
    ...
    {
        public static IServiceCollection AddFileService(this IServiceCollection services, Microsoft.Extensions.Configuration.IConfiguration configuration)
        {
          

            services.AddDbContextPool<FileSerDbContext>(options =>
            {
                options.UseMySQL(GetConnectionString(configuration, DatabaseType.MySQL));// 可切换数据库源UseOracle,UseSqlserver
            }, poolSize: 64);

            services.AddTransient<IDbContext, FileSerDbContext>();
